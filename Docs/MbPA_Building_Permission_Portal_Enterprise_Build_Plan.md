...
> **Amendment (PoC-parity pass):** the legacy Google Apps Script PoC (`Legacy PoC Codebase/`) received updates after this plan's frontend work was first built — OTP entry masked (`type="password"` + `autocomplete="one-time-code"`), a request-timeout-and-reconnect-recovery layer around every API call, an explicit "final approval" vs. "next stage" distinction on the officer's approve action, and the applicant-side "raise a complaint" flow (PRD §10.11/DRD §13 — `Complaint` already existed as an API-only surface; no React UI called it). All four were reviewed against the rebuilt system and closed: `Login.tsx`/`Signup.tsx` OTP inputs now match the masked-input pattern; `frontend/src/lib/api.ts`'s `fetchWithTimeout()` (wired into both the typed `openapi-fetch` client and the legacy `api()`/`uploadFile()` helpers) bounds every request at 65s and, on the officer's milestone-action submit specifically, re-checks the queue rather than reporting a false failure if the connection merely dropped after the transition committed; `OfficerQueueItemSerializer.is_final_step` (backed by a `Subquery` for the stream's max `StreamMilestone.sequence`) drives the button-label distinction; and `ApplicantDashboard.tsx` gained a "Raise a Complaint" tab wired to the existing `apps/compliance` endpoints. Fixing the complaint UI's ownership gap on the way (any authenticated user could previously raise a complaint against any application by guessing its id) also closed a real IDOR-shaped hole in `ComplaintListCreateView.post`.
>
> While verifying this in a real browser (not just against the test suite), two further gaps surfaced and were fixed alongside it: `Login.tsx` never redirected officers to `/officer` after OTP verification — every officer login landed on the applicant dashboard, with `/officer` reachable only by typing the URL — and `config/settings/local.py` had no `CSRF_TRUSTED_ORIGINS` for the documented `npm run dev` (port 5173) → Django (port 8000) proxy setup, so any mutating request made through the actual local dev flow in a real browser 403'd on Django's Origin check regardless of a valid CSRF cookie. Neither was caught by the test suite, since Django's test client doesn't enforce the browser-side Origin check the same way.
